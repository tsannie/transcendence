...
export class ChannelController {
	...
	@UseGuards( JwtTwoFactorGuard )
	@Post('banUser')
	async banUser(@Body() ban_request: ChannelActionsDto, @Request() req) : Promise<BanEntity> {
		return await this.channelService.banUser(ban_request, req.user);
	}

	@UseGuards( JwtTwoFactorGuard )
	@Post('unBanUser')
	async unBanUser(@Body() ban_request: ChannelActionsDto, @Request() req) : Promise<BanEntity> {
	...
	@UseGuards( JwtTwoFactorGuard )
	@Post('muteUser')
	async muteUser(@Body() channel: ChannelActionsDto, @Request() req) : Promise<MuteEntity>{
		return await this.channelService.muteUser(channel, req.user);
	}

	@UseGuards( JwtTwoFactorGuard )
	@Post('unmuteUser')
	async unMuteUser(@Body() channel: ChannelActionsDto, @Request() req) : Promise<MuteEntity>{
