...
export class BanMuteService {
  ...
  async generate<T extends MuteEntity | BanEntity>(
    c: new () => T,
    duration: number,
    ...
    } else new_action.end = null;

    if (new_action instanceof BanEntity)
      return await this.banRepository.save(new_action);
    else return await this.muteRepository.save(new_action);
  }

  async remove(c: BanEntity | MuteEntity): Promise<BanEntity | MuteEntity> {
    if (c instanceof BanEntity) return await this.banRepository.remove(c);
    else return await this.muteRepository.remove(c);
  }

  async isMuted(
    channel: ChannelEntity,
    user: UserEntity,
  ): Promise<MuteEntity | boolean> {
    ...
    if (request) {
      if (!this.checkDuration(request)) return true;
      else return (await this.remove(request)) as MuteEntity;
    } else return false;
  }
  ...
  checkDuration(ban_mute: BanMuteEntity): boolean {
    ...
    if (ban_mute.end && current_date >= ban_mute.end) return true;
    else return false;
  }
}
