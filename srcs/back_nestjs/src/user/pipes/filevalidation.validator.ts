...
export class AvatarFormatValidator extends FileValidator<AvatarFormatValidatorOptions> {
  private _isjpeg(buffer: Buffer): boolean {
    ...
    if (
      magic_number.slice(0, 8) === jpeg[0] ||
      magic_number.slice(0, 8) === jpeg[1] ||
      magic_number.slice(0, 8) === jpeg[2] ||
  ...
  private _ispng(buffer: Buffer): boolean {
    ...
    if (magic_number === png) return true;
    else return false;
  }
  ...
  private _checkFormat(buffer: Buffer): boolean {
    ...
    if (
      this.validationOptions.format.includes('jpeg') ||
      this.validationOptions.format.includes('jpg')
    )
      jpeg_bool = this._isjpeg(buffer);
    if (this.validationOptions.format.includes('png'))
      png_bool = this._ispng(buffer);
    if (!jpeg_bool && !png_bool) return false;
    return true;
  }
  ...
  public isValid(file: Express.Multer.File): boolean {
    ...
    if (!this._checkExtension(file.originalname)) return false;
    if (!this._checkFormat(file.buffer)) return false;
    return true;
  }
