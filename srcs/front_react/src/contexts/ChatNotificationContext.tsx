...
export const ChatNotifContext = React.createContext<INotifContext>(
  {} as INotifContext
);
...
const ChatNotifProvider = ({ children }: ChatNotifProvider) => {
  ...
  const addChannel = (convId: string) => {
    if (channels.find((channelId) => channelId === convId)) return;
  ...
  return (
    <ChatNotifContext.Provider
      value={{ channels, addChannel, removeChannel } as INotifContext}
    >
      {children}
    </ChatNotifContext.Provider>
  );
