...
function UserOptions(props: IMemberProps) {
  ...
  const adminOptions = () => {
    ...
    if (isOwner) {
      ...
      adminOptionsJSX.push(
        <button key={key++} onClick={revokeAdmin}>
          unAdmin
        </button>
      );
      adminOptionsJSX.push(
  ...
  const memberOptions = () => {
    ...
    if (isOwner)
      adminOptionsJSX.push(
        <button key={key++} onClick={makeAdmin}>
          make Admin
        </button>
      );
    if (isOwner || isAdmin) {
  ...
  const mutedOptions = () => {
    ...
    if (isAdmin || isOwner) {
      adminOptionsJSX.push(
        <button key={key++} onClick={unMuteUser}>
          Unmute
        </button>
      );
      adminOptionsJSX.push(
  ...
  const bannedOptions = () => {
    ...
    if (isAdmin || isOwner) {
      adminOptionsJSX.push(
        <button key={key++} onClick={unBanUser}>
          UnBan
        </button>
      );
    }
  ...
  useEffect(() => {
    const closeDropdown = (e: any) => {
      if (!e.composedPath().includes(buttonRef.current)) setOpen(false);
    };
  ...
  const handleButtonClick = (
    ...
    const rec = buttonRef.current
      ?.closest("div.conversation__options__members")
